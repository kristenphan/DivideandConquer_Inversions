...
def merge(arr1, arr2):
    ...
    """
    # create an empty array, an inversion count,
    # and a count of the number of elements removed from arr2 during the merging process
    arr_sorted = []
    inv_count = 0
    arr2_removal_count = 0
    ...
    while arr1 and arr2:
        ...
        if a > b: # an inversion is found in current pair (a, b)
            ...
            # no inversion
            # Because arr1 is sorted, for any element x in arr1 that comes after a, x > a,
            # implying x is greater than all elements in arr2 which were previously removed
            # Therefore, increment the inv_count by the number of elmenets in arr2 previously removed
            if len(arr1) != 1:
